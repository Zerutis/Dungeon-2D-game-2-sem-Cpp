...
Mapp::Mapp(const std::string mapfile) : tileset(nullptr) {
	map = new int* [Map_Tile_Height];
	for (int i = 0; i < Map_Tile_Height; i++) {
		map[i] = new int[Map_Tile_Width];
	}
...
Mapp::Mapp(const Mapp& oldOne) : tileset(nullptr) {
	map = new int* [Map_Tile_Height];
	for (int i = 0; i < Map_Tile_Height; i++) {
		map[i] = new int[Map_Tile_Width];
	}
...
void Mapp::loadMap(const std::string filename, SDL_Renderer* ren) {
...
void Mapp::drawMap(SDL_Renderer* ren) const {
...
bool Mapp::moveable (const int x, const int y) const  {
	int i = (y+16) / 32; // þemyn
...
bool Mapp::isFinish(const vec2D<int> hero_pos) const {
	if ((hero_pos.xpos == finish.xpos) && ((hero_pos.ypos > finish.ypos) || (hero_pos.ypos < finish.ypos + 32.0))) {
		return true;
	}
	else
		return false;
